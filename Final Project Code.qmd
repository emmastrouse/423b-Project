...
5.  Following what we did in lab 4.2, create plots of predicted mobility as a function of the value of predictors. In each case, be sure to use appropriate values of the variable you are investigating and hold all other variables at their mean value. Interpret each of these plots. What is the ensemble showing you about the relationships between each of these variables and happiness?

```{r}
d <- d |> 
  mutate(type = 'data')

range(d$beer_per_capita)
#check out the distribution of beer variable to choose reasonable range
ggplot(data = d, aes(x = beer_per_capita)) +
  geom_histogram()


pred_beer <- data.frame(beer_per_capita=seq(1, 376, length.out=115),
                   population=mean(d$population),
                   infant_mortality_per_thousand = mean(d$infant_mortality_per_thousand),
                   wine_per_capita = mean(d$wine_per_capita),
                   gdp = mean(d$gdp),
...
gam_fit <- fit(final_gam, data=d)

pred_beer <- bind_cols(pred_beer, predict(gam_fit, new_data=pred_beer)) |> 
  rename(happiness = .pred)



bind_rows(d, pred_beer) |> 
  ...
  ggplot(aes(x=beer_per_capita, y=happiness, size=type, color=type)) + 
  geom_point() + 
  scale_color_manual(values=c('data'='black', 
                              'model'='cornflowerblue')) +
  scale_size_manual(values=c('data'=2,
                             'model'=0.3)) +
  labs(x = 'Beer Per Capita', y = 'Happiness', title = 'Beer Per Capita vs Happiness\nGAM vs Data')+
  theme_bw()
